<html>
    <head>
        <title>Scryptonite | Firebase Cloud Messaging</title>
        <meta charset="UTF-8">
    ...
    <body>
        ...
        if(isset($_POST['addToDb'])) {
	        ...
	        $time = date('y-m-d H:i');
	        $time = '20'.$time;
	        $stmt = $user->runQuery("INSERT INTO notification (content, timestamp) VALUES (:content, :ts) ");
	        $stmt->bindParam(":content", $content, PDO::PARAM_STR);
	        $stmt->bindParam(":ts", $time, PDO::PARAM_STR);
	        $stmt->execute();
        }
 ...
 -->
            <form class="pure-form pure-form-stacked" method="post">
                <fieldset>
                    ...
                    <input type="text" id="title1" name="title" class="pure-input-1-2" placeholder="Enter title" required>

                    <label for="message1">Message</label>
                    <textarea class="pure-input-1-2" name="message" id="message1" rows="5" placeholder="Notification message!" required></textarea>

                    <!-- <label for="include_image1" class="pure-checkbox">
                        <input id="include_image1" name="include_image" type="checkbox" checked> Add to database
                    </label> -->

                    <label for="include_image1" class="pure-checkbox">
                        <input id="include_image1" name="addToDb" type="checkbox" checked> Add to database
                    </label>
